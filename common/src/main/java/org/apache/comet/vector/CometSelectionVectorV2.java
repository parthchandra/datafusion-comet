 ...
 */
public class CometSelectionVectorV2 extends CometStructVector {
   ...
   */
  public CometSelectionVectorV2(CometVector originalVector, int[] selectionIndices) {
    super(
  ...
  /**
   * Creates a nested selection by applying additional indices to this selection vector, returning
   * the same type as this vector.
   *
   * @param indices The indices to select from this selection vector
   * @return A new CometSelectionVectorV2 representing the nested selection
   * @throws IllegalArgumentException if any index is out of bounds
   */
  public CometSelectionVectorV2 takeV2(int[] indices) {
    int[] newIndices = new int[indices.length];
    for (int i = 0; i < indices.length; i++) {
      int idx = indices[i];
      if (idx < 0 || idx >= numValues) {
    ...
    return new CometSelectionVectorV2(originalVector, newIndices);
  }
  ...
  public CometVector slice(int offset, int length) {
    ...
    return new CometSelectionVectorV2(originalVector, slicedIndices);
  }
