...
use jni::{
    ...
    objects::{
        JByteArray, JClass, JDoubleArray, JIntArray, JLongArray, JObject, JObjectArray,
        JPrimitiveArray, JString, ReleaseMode,
    },
...
use jni::{
    objects::GlobalRef,
    sys::{jboolean, jdouble, jdoubleArray, jintArray, jobjectArray, jsize, jstring},
};
...
/// Exports a Java int array to Rust and returns the memory address where it's stored.
/// The array is copied to Rust-managed memory and must be freed using `freeExportedIntArray`.
/// # Safety
/// This function is inherently unsafe since it deals with raw pointers passed from JNI.
#[no_mangle]
pub unsafe extern "system" fn Java_org_apache_comet_Native_exportIntArray(
    e: JNIEnv,
    _class: JClass,
    java_array: jintArray,
) -> jlong {
    try_unwrap_or_throw(&e, |mut env| {
        let array = JIntArray::from_raw(java_array);
        let array_len = env.get_array_length(&array)? as usize;

        // Get array elements from Java
        let elements = env.get_array_elements(&array, ReleaseMode::NoCopyBack)?;
        let java_slice = elements.as_ptr();

        // Copy to Rust-managed memory
        let mut rust_vec = Vec::with_capacity(array_len);
        std::ptr::copy_nonoverlapping(java_slice, rust_vec.as_mut_ptr(), array_len);
        rust_vec.set_len(array_len);

        // Box the vector and return its pointer
        let boxed_vec = Box::new(rust_vec);
        let ptr = Box::into_raw(boxed_vec) as *mut Vec<i32>;

        Ok(ptr as jlong)
    })
}

/// Exports a Java long array to Rust and returns the memory address where it's stored.
/// The array is copied to Rust-managed memory and must be freed using `freeExportedLongArray`.
/// # Safety
/// This function is inherently unsafe since it deals with raw pointers passed from JNI.
#[no_mangle]
pub unsafe extern "system" fn Java_org_apache_comet_Native_exportLongArray(
    e: JNIEnv,
    _class: JClass,
    java_array: jlongArray,
) -> jlong {
    try_unwrap_or_throw(&e, |mut env| {
        let array = JLongArray::from_raw(java_array);
        let array_len = env.get_array_length(&array)? as usize;
        ...
        // Box the vector and return its pointer
        let boxed_vec = Box::new(rust_vec);
        let ptr = Box::into_raw(boxed_vec) as *mut Vec<i64>;

        Ok(ptr as jlong)
    })
}

/// Exports a Java double array to Rust and returns the memory address where it's stored.
/// The array is copied to Rust-managed memory and must be freed using `freeExportedDoubleArray`.
/// # Safety
/// This function is inherently unsafe since it deals with raw pointers passed from JNI.
#[no_mangle]
pub unsafe extern "system" fn Java_org_apache_comet_Native_exportDoubleArray(
    e: JNIEnv,
    _class: JClass,
    java_array: jdoubleArray,
) -> jlong {
    try_unwrap_or_throw(&e, |mut env| {
        let array = JDoubleArray::from_raw(java_array);
        let array_len = env.get_array_length(&array)? as usize;
        ...
        // Box the vector and return its pointer
        let boxed_vec = Box::new(rust_vec);
        let ptr = Box::into_raw(boxed_vec) as *mut Vec<f64>;

        Ok(ptr as jlong)
    })
}

/// Exports a Java string array to Rust and returns the memory address where it's stored.
/// The array is copied to Rust-managed memory and must be freed using `freeExportedStringArray`.
/// # Safety
/// This function is inherently unsafe since it deals with raw pointers passed from JNI.
#[no_mangle]
pub unsafe extern "system" fn Java_org_apache_comet_Native_exportStringArray(
    e: JNIEnv,
    _class: JClass,
    java_array: jobjectArray,
) -> jlong {
    try_unwrap_or_throw(&e, |mut env| {
        let array_len = env.get_array_length(&JObjectArray::from_raw(java_array))? as usize;
        let mut rust_vec = Vec::with_capacity(array_len);

        // Convert each Java string to Rust String
        for i in 0..array_len {
            let java_string =
                env.get_object_array_element(&JObjectArray::from_raw(java_array), i as jsize)?;
            if java_string.is_null() {
                rust_vec.push(String::new()); // Empty string for null
            } else {
                let jstring = JString::from_raw(java_string.into_raw());
                let rust_string: String = env.get_string(&jstring).unwrap().into();
                rust_vec.push(rust_string);
            }
        }

        // Box the vector and return its pointer
        let boxed_vec = Box::new(rust_vec);
        let ptr = Box::into_raw(boxed_vec) as *mut Vec<String>;

        Ok(ptr as jlong)
    })
}

/// Frees a Rust-managed int array that was exported from Java.
/// # Safety
/// This function is inherently unsafe since it deals with raw pointers.
/// The pointer must be valid and must have been created by `exportIntArray`.
#[no_mangle]
pub unsafe extern "system" fn Java_org_apache_comet_Native_freeExportedIntArray(
    _e: JNIEnv,
    _class: JClass,
    ptr: jlong,
) {
    if ptr != 0 {
        let vec_ptr = ptr as *mut Vec<i32>;
        let _boxed_vec = Box::from_raw(vec_ptr);
        // Box is automatically dropped here, freeing the memory
    }
}

/// Frees a Rust-managed long array that was exported from Java.
/// # Safety
/// This function is inherently unsafe since it deals with raw pointers.
/// The pointer must be valid and must have been created by `exportLongArray`.
#[no_mangle]
pub unsafe extern "system" fn Java_org_apache_comet_Native_freeExportedLongArray(
    _e: JNIEnv,
    _class: JClass,
    ptr: jlong,
) {
    if ptr != 0 {
        let vec_ptr = ptr as *mut Vec<i64>;
        let _boxed_vec = Box::from_raw(vec_ptr);
        // Box is automatically dropped here, freeing the memory
    }
}

/// Frees a Rust-managed double array that was exported from Java.
/// # Safety
/// This function is inherently unsafe since it deals with raw pointers.
/// The pointer must be valid and must have been created by `exportDoubleArray`.
#[no_mangle]
pub unsafe extern "system" fn Java_org_apache_comet_Native_freeExportedDoubleArray(
    _e: JNIEnv,
    _class: JClass,
    ptr: jlong,
) {
    if ptr != 0 {
        let vec_ptr = ptr as *mut Vec<f64>;
        let _boxed_vec = Box::from_raw(vec_ptr);
        // Box is automatically dropped here, freeing the memory
    }
}

/// Frees a Rust-managed string array that was exported from Java.
/// # Safety
/// This function is inherently unsafe since it deals with raw pointers.
/// The pointer must be valid and must have been created by `exportStringArray`.
#[no_mangle]
pub unsafe extern "system" fn Java_org_apache_comet_Native_freeExportedStringArray(
    _e: JNIEnv,
    _class: JClass,
    ptr: jlong,
) {
    if ptr != 0 {
        let vec_ptr = ptr as *mut Vec<String>;
        let _boxed_vec = Box::from_raw(vec_ptr);
        // Box is automatically dropped here, freeing the memory
    }
}
