...
//! Query context registry for error reporting
//!
...
use datafusion_comet_spark_expr::QueryContext;
...
lazy_static::lazy_static! {
    static ref GLOBAL_REGISTRY: Arc<QueryContextRegistry> = Arc::new(QueryContextRegistry::new());
}
...
mod tests {
    ...
    fn test_registry_replace() {
        ...
        let retrieved = registry.get(1).unwrap();
        assert!(retrieved.sql_text.contains("SELECT b"));
    }
    ...
    fn test_arc_sharing() {
        ...
        let retrieved1 = registry.get(1).unwrap();
        let retrieved2 = registry.get(1).unwrap();
